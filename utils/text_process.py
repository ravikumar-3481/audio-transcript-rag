...
class GetTranscript:
    ...
    def get_transcript(self,source : str , language : str = "english") -> str:
        if not source:
...
def process_transcript(source : str = None, language : str = "english") -> str:
    try:
        raw_transcript = GetTranscript().get_transcript(source=source, language=language)
    except Exception as e:
    ...
    fs.save_to_dir(cleaned, DIRECTORY = "data/transcripts", filename = "processed_text.txt")
    return cleaned
